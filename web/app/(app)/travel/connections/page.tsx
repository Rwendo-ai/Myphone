...
// Traveller community feed — 18+ only, same gate as the mobile app
// (conservative: unknown DOB counts as under-18).
export default async function ConnectionsPage() {
  ...
  if (age === null || age < 18 || p?.is_minor === true) redirect('/travel');
  ...
  const defaultName = user.email?.split('@')[0] ?? 'Traveller';

  return (
    <main className="px-4 pt-8 pb-6 max-w-2xl mx-auto space-y-4">
      <header>
        <Link href="/travel" className="text-white/60 hover:text-white text-sm">← Travel</Link>
        <h1 className="text-3xl font-extrabold mt-2">Connections</h1>
